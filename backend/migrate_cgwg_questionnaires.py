...
def migrate_cgwg_questionnaires():
    ...
    for r_data in responses_data:
        ...
        contact_person = extract_answer_value(r_data.get("answers", []), "name_of_person")
        
        # Calculate expiry (30 days from created_at or now)
        created_at = parse_datetime(r_data.get("created_at"))
        expires_at = created_at + timedelta(days=30) if created_at else timezone.now() + timedelta(days=30)
        ...
        token = SAQToken.objects.create(
            ...
            status=SAQToken.Status.SUBMITTED if r_data.get("is_submitted") else SAQToken.Status.PENDING,
    ...
    for a_data in answers_data:
        ...
        response = SAQResponse.objects.create(
            token=token,
            question=question,
