...
from knowledge.services import ChunkResult, embed_and_store, get_embedding_dimension, get_embedding_model


def test_query_similar_evidence_treats_missing_pinecone_index_as_no_matches(monkeypatch):
    def missing_index():
        raise PineconeApiException(status=404, reason="Resource sustainability-ai not found")

    monkeypatch.setattr("assessments.services.validation.get_pinecone_index", missing_index)

    assert query_similar_evidence([0.1, 0.2, 0.3], "org-1") == []


def test_huggingface_embedding_provider_uses_endpoint_not_local_sentence_transformers(settings):
...
def test_embed_and_store_creates_index_with_configured_embedding_dimension(settings, monkeypatch):
    created_indexes = []
    upserted_vectors = []
    ...
    monkeypatch.setattr("knowledge.services.get_pinecone_client", lambda: FakePinecone())
    monkeypatch.setattr("knowledge.services.get_embedding_model", lambda: FakeEmbeddings())
