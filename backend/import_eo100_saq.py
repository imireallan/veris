...
def import_eo100_saq():
    ...
    print(f"  Total Questions: {AssessmentQuestion.objects.filter(template__framework=framework).count()}")
    print(f"\n  Supplements:")
    for supplement_type, _ in supplements.values():
        count = AssessmentTemplate.objects.filter(
            framework=framework,
            supplement_type=getattr(AssessmentTemplate.SupplementType, supplement_type, None)
        ).count()
