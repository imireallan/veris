...
class ReportGenerator:
    ...
    def _render_html(self) -> str:
        ...
        template_path = Path(settings.BASE_DIR) / "reports" / "templates" / "report.html"
    ...
    def _get_template_context(self) -> dict:
        ...
        for f in findings:
            findings_data.append({
                ...
                "severity": f.severity,
                "get_severity_display": f.get_severity_display() if hasattr(f, "get_severity_display") else f.severity,
                "status": getattr(f, "status", "OPEN"),
                "get_status_display": f.get_status_display() if hasattr(f, "get_status_display") else getattr(f, "status", "OPEN"),
            })
        ...
        for cycle in cip_cycles:
            cip_cycles_data.append({
                ...
                "get_status_display": cycle.get_status_display() if hasattr(cycle, "get_status_display") else getattr(cycle, "status", "N/A"),
            })
        ...
        context = {
            ...
            # Organization
            "organization_name": assessment.organization.name if hasattr(assessment, 'organization') else "Organization",
            
            # Report sections
            "executive_summary": report.executive_summary or "No executive summary provided.",
            "methodology": report.methodology or "Standard ESG assessment methodology applied.",
            ...
            # Generated date
            "generated_at": assessment.updated_at.strftime("%B %d, %Y") if assessment.updated_at else "N/A",
        }
